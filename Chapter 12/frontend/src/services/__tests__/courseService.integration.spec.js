...
describe('CourseService Integration Tests', () => {
  ...
  /**
   * PATTERN 1: Get Courses List
   * Contract: GET /api/courses/?page=1
   * Success: 200 OK with paginated course list
  ...
  describe('Get Courses List', () => {
    ...
    it('handles pagination with next/previous links', async () => {
      ...
      expect(response.data.next).toBeDefined() // Has more pages
      expect(response.data.previous).toBeNull() // First page
    })
  ...
  /**
   * PATTERN 2: Get Course Detail
   * Contract: GET /api/courses/:id/
   * Success: 200 OK with full course details
  ...
  /**
   * PATTERN 3: Enroll in Course
   * Contract: POST /api/courses/:id/enroll/
   * Success: 201 Created with enrollment object
   * Errors: 401 (not authenticated), 404 (course not found), 409 (already enrolled)
   */
  ...
  /**
   * PATTERN 4: Get Course Reviews
   * Contract: GET /api/courses/:id/reviews/
   * Success: 200 OK with reviews list
   */
  ...
  /**
   * PATTERN 5: Complete course enrollment flow
   * List -> Detail -> Enroll -> Verify
   */
  describe('Complete Course Enrollment Flow', () => {
    it('executes full enrollment workflow', async () => {
      // Step 1: Get courses list
      mockApiClient.get.mockResolvedValueOnce({
        status: 200,
